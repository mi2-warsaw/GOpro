...
expr <- prepareData(RTCGA = TRUE, cohorts = c('lung adenocarcinoma', 'endometrioid', 'lung squamous carcinoma', 'leukemia'))
```
...
## Example 
Genes for which any observation is not available data are omitted to simplify this example. Alternatively, imputation techinques might be used to replace NA observations.
Genes aliases must be used as genes names and they must be arranged in the same order for each medical group. Genes must be stored in columns and patients (samples) in rows. Data frames of genes and patients observations must be arranged into a list. The length of list should be consistent with the number of different medical groups.
...
tukey.results[[3]] 
```
The next step is to find all GO terms annotated with each of the gene groups. The *min* and *max* parameter denote the range of counts of genes annotated with each GO term. All GO terms with counts above or below this range are omitted from the analysis. \bigskip
...
cluster.top[1:3]
```
The results of the analysis can be presented in a more descriptive way or in a concise one. \bigskip
The concise way is obtained by using \textit{descGO} function (additionally information about p-values returned by *findTopGOs* are included). To get more descriptive results use \textit{extendGO} function.\bigskip
...
```{r, echo=TRUE, fig.height=10, fig.width=10, message = FALSE, results='hide', cache = TRUE}
