...
#'
#' @param extendedGO A data frame with a one column of group and another column with GO term (returned by \code{extendGO}).
#'
#' @return A data frame with the most frequently occurring ontologies in each group.
#'
#' @examples
...
ontologyOver  <- function (extendedGO)
{
  ...
  counted.onto <- cbind.data.frame(group = seq_len(no), onto = rep(c('BP', 'CC', 'MF'), each = no), counts = c(BP, CC, MF))
