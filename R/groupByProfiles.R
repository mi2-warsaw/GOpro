...
#'
#' @param part.tukey.res A vector of a list of a result returned by \code{TukeyHSDTest}.
#' @param sig.level A numeric significance level of a test.
#' @param n A number of compared groups.
#'
#' @return A character vector of a name denoting results of all pairwise Tukey comparisons.
...
makeNames <- function(part.tukey.res, sig.level, n)
{
  ...
  if(IsConflict(part.tukey.res, sig.level, n))
  {
    ...
    paste(c(paste(c(signedNames, "ascending means: "), collapse = ";"), paste(names(sort(means)), collapse = ",")), collapse = "")
  } else {
    name <- names(means)[1]
    ...
    if(length(greaternames) == 1)
    {
      ...
      {
        temp  <- unlist(strsplit(x, "-"))
        if(length(which(greaternames == temp[1])) == 0 || length(which(greaternames == temp[2])) == 0)
        {
          FALSE
        } else {
          TRUE
      ...
      output <- paste(c(output, makeNames(c(greatermeans, pvals[greaterpvals]), sig.level, length(greatermeans))), collapse = "<")
    }
    if(length(lowernames) == 1)
    {
      ...
      {
        temp  <- unlist(strsplit(x, "-"))
        if(length(which(lowernames == temp[1])) == 0 || length(which(lowernames == temp[2])) == 0)
        {
          FALSE
        } else {
          TRUE
      ...
      output <- paste(c(makeNames(c(lowermeans, pvals[lowerpvals]), sig.level, length(lowermeans)), output), collapse = "<")
    }
    output
  }
}

#' Check if there is a conflict between results of the Tukey test, ex. a=b, b=c, c!=a.
#'
#' @param part.tukey.res A vector of a list of a result returned by \code{TukeyHSDTest}.
#' @param sig.level A numeric significance level of a test.
#' @param n A number of compared groups.

IsConflict <- function (part.tukey.res, sig.level, n)
{
  means <- part.tukey.res[1:n]
  pvals <- part.tukey.res[(n + 1):length(part.tukey.res)]
  ...
  for (i in 1:(n - 1))
  {
    ...
    for (j in (i + 1):n)
    {
      ...
      pair_str1 <- paste(c(group1,group2), collapse = "-")
      pair_str2 <- paste(c(group2,group1), collapse = "-")
      pair_position <- max(which(names(pvals) == pair_str1), which(names(pvals) == pair_str2))
      ...
      for (k in 1:n)
      {
        if (i != k && j != k)
        {
          ...
          pair_position2 <- max(which(names(pvals) == pair_str1),which(names(pvals) == pair_str2))

          pair_str1 <- paste(c(group2,group3), collapse = "-")
          pair_str2 <- paste(c(group3,group2), collapse = "-")

          pair_position3 <- max(which(names(pvals) == pair_str1), which(names(pvals) == pair_str2))

          if((pvals[pair_position] > sig.level))
          {
            if((pvals[pair_position2] < sig.level  && pvals[pair_position3] > sig.level)
               || (pvals[pair_position2] > sig.level  && pvals[pair_position3] < sig.level)
               || ((pvals[pair_position2] < sig.level  && pvals[pair_position3] < sig.level)
                   && ((mean1 > mean3 && mean2 < mean3) || (mean1 < mean3 && mean2 > mean3))))
            {
              return(TRUE)
            }
          }
...
#' Genes grouped by profiles determined by the result returned from \code{tukeyHSDTest} function.
#'
...
#' @param sig.level A numeric value, a significance level used in Tukey's all pairwise comparisons (\code{groupByTukey}).
#'
...
{
  n <- length(expr)
  named.profiles  <- sapply(tukey.results, function (x) makeNames(x, sig.level, n))
  groups.names <- sort(unique(named.profiles))
  result.groups <- lapply(groups.names, function(x) row.names(expr[[1]][which(named.profiles == x), ]))
