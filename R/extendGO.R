...
#'
#' @return A data frame with an extended description (term, definition, and ontology) of gene ontology terms.
#'
#' @examples
...
extendGO  <- function (TopGOs)
{
  ...
  GO.descr <- lapply(TopG, function (GOID)
  {
    if(!is.na(GOID))
    {
      ...
      data.frame(cbind(GOID = NA, TERM = NA, DEFINITION = NA, ONTOLOGY = NA))
    }
  })
  gnames <- seq_along(group.size)
  assignedFunctions <- data.frame(rep(gnames, group.size), matrix(unlist(GO.descr), ncol = 4, byrow = TRUE))
  names(assignedFunctions) <- c("GROUP", "GO ID", "TERM", "DEFINITION", "ONTOLOGY")
  assignedFunctions
}
