...
#'
#'@param groups a list of data frames of gene expressions with genes in rows and samples in columns.
#' rows named with genes' aliases. The order of samples and genes has to be the same for each data frame.
#'@param topAOV A numeric value, a number of most significantly differentiated genes to be returned.
#'@param sig.levelAOV a numeric value, a significance level used in BH correction for multiple testing (\code{aovTopTest}).
#'@param sig.levelGO A numeric value, a significance level used in BH correction for multiple testing (\code{findTopGOs}).
#'@param minGO A minimum number of functions that a gene needs to represent to be considered as frequent.
#'@param maxGO A maximum number of functions that a gene needs to represent to be considered as frequent.
#'@param parallel A logical value indicating if a task should be run on more than one core.
#'@param clust.metric The method to calculate a distance measure used in hierarchical clustering, possible names: "euclidean",
#'"maximum", "manhattan", "canberra", "binary" or "minkowski".
#'@param clust.method The agglomeration method to used to cluster genes. This should be #'one of "ward.D",
#'"ward.D2", "single", "complete", "average" (= UPGMA), "mcquitty" (= WPGMA), "median" (= WPGMC) or "centroid" (= UPGMC).
#'@param topGO A number of the most characteristic functions of groups of genes to be returned.
#'@param grouped A method of grouping genes, one of 'tukey' and 'clustering'.
#'@param dist.matrix A matrix with calculated distances to be used as a metric by \code{hclust} function.
                     ...
                     topGO = 3, sig.levelTUK = 0.05, onto = c('MF', 'BP', 'CC'), extend = FALSE)
{
  ...
  {
    cluster.results <- clustering(aov.results, clust.metric, clust.method, dist.matrix)
    all.gos <- findAllGOs(cluster.results, geneUni = rownames(groups[[1]]))
