...
#'
#' @param expr A named list of data frames of significantly different gene expressions with genes in rows and samples in columns.
#' rows should be named with genes' aliases. The order of genes has to be the same for each data frame. The names of list elements should refer to groups of different cases.
...
#'
#' @return A data frame with groups of genes named by profiles. Profiles are determined by the differences between groups.
#'
#' @examples
...
{
  if(!all(sapply(expr, function(x) sapply(expr, function(y) row.names(x) == row.names(y)))))
  {
